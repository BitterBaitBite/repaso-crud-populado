...
const { isEmptyCoaster } = require('../middleware');
...
router.post('/new', isEmptyCoaster, (req, res) => {
	// Prueba postman
	// res.json(req.body);
...
router.get('/:id/editar', (req, res) => {
	Coaster.findById(req.params.id)
		.then((coaster) => {
			Park.find()
				.then((parks) => {
					selParks = parks.map((park) =>
						String(park._id) == String(coaster.park_id) ? { park, selected: true } : { park, selected: false }
					);
					res.render('pages/coasters/edit-coaster', { coaster, parks: selParks });
				})
				.catch((err) => console.error(err));
		})
		.catch((err) => console.error(err));
...
router.post('/:id/editar', isEmptyCoaster, (req, res) => {
